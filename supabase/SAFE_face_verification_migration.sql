-- ============================================
-- SAFE FACE VERIFICATION MIGRATION
-- This will NOT break existing functionality
-- ============================================

-- Add face verification columns to exam_attempts (all optional with defaults)
ALTER TABLE exam_attempts
ADD COLUMN IF NOT EXISTS reference_face_url TEXT DEFAULT NULL,
ADD COLUMN IF NOT EXISTS reference_face_embedding FLOAT8[] DEFAULT NULL,
ADD COLUMN IF NOT EXISTS face_verified BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS face_verification_at TIMESTAMP WITH TIME ZONE DEFAULT NULL,
...
ADD COLUMN IF NOT EXISTS face_consent_at TIMESTAMP WITH TIME ZONE DEFAULT NULL;

-- Create face_enrollments table (new table, won't affect existing data)
...
-- Create face_snapshots table (new table, won't affect existing data)
...
-- Add comments (documentation only, no data changes)
...
-- Check new tables exist (read-only check)
SELECT table_name 
